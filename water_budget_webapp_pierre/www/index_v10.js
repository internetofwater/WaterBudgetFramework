...
function (message) {
    ...
    d3.csv("hyperlink3.csv").then(function(uri_data) {
...
Shiny.addCustomMessageHandler("data_source_height",
function(message) {
    leaf_nodes_3 = message;
})

Shiny.addCustomMessageHandler("data_source_json",
function (message) {
    ...
    d3.csv("hyperlink3.csv").then(function(uri_data) {
        ...
        addURI(ds_uri_unique, 'ds', 'dsL', data);
            data.children.forEach(item1 => {
                ...
                item1.children.forEach(item2 =>{
                    addURI(em_uri_unique, 'em', 'emL', item2);
                    item2.children.forEach(item3 => {
                        addURI(c_uri_unique, 'c', 'cL', item3);
                        // item3.children.forEach(item4 => {
                        //     addURI(c_uri_unique, 'c', 'cL', item4);
                        // })
                    })
                })
            })
        ...
        // appending svg object to the body div "sticky"
        var svg_sticky = d3.select("#data_source_sticky")  
            .append("svg")
                .attr("width", width + margin.right + margin.left) //to extend white background of label levels
                .attr("height", 40)
            .append("g")
        ...
        function update(source) {
            ...
            // Adding labels for each level
            // using underscore.js library
            var level_labels = ["Data Source", "Parameter", "Estimation Method", "Component"]
            var depthOrder = _.uniq(_.pluck(nodes, "depth")).sort();
            var label_data = [];
            for (var n in depthOrder){
                label_data[n] = {
            ...
            levels.data(label_data)
                ...
                // .attr("x", -40)
                .attr("x", function(d) {
                    return d.label === "Estimation Method" ? 200 : 220;})
                .attr("y", 0)
                .attr("font-family","arial")
                .style("font-weight", "bold")
                .style("fill", "#777777")
        ...
        */                  path = "M" + s.y + "," + s.x
        ...
        function autoscroll() {
            d3.select("#data_source_container")
                .transition()
                .duration(1000)
                .tween("scroll", scrollTween((document.body.getBoundingClientRect().height - window.innerHeight)/2 + 150));
