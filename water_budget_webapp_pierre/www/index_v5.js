...
function (message) {
    ...
    var width = 1400 - margin.left - margin.right //change width because leaf nodes were going out
    var height = (leaf_nodes*24) - margin.top - margin.bottom; //1200 height for about 40 leaf nodes
    ...
    function update(source) {
        ...
        levelRect = levelSVG.append("rect")
        levels.data(depthOrder)
            .enter().append("g")
            .attr("class", "level")
            .attr("transform", function(d) { return "translate(" + d*235 + "," + 0 + ")"; })
            .append("text")
            .text(function(d){
                if (d === 0){
                    if (label_level_1 === 1){
    ...
    */                  path = "M" + s.y + "," + s.x
        ...
        function click(d) {
            if (d.children) {
                d._children = d.children;
                d.children = null;
                console.log(depthOrder.length);
                console.log(label_level_3);
    ...
    function autoscroll() {
        d3.select("#container")
            .transition()
            .duration(1000)
            .tween("scroll", scrollTween((document.body.getBoundingClientRect().height - window.innerHeight)/2));
