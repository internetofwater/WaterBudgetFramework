...
# Summary of component
  observeEvent(input$runButton1, {
    ...
    summary_title <- paste(input$components, input$states1, 
                           sep = "-")
    properties_list <- paste("summary", properties, sep="_")
    for (i in 1:length(properties)) {
      assign(paste(properties_list[i]), 
    ...
    lapply(1:length(properties_display), function(i){
      output[[properties[i]]] <- renderText(paste(properties_display[i], get(properties_list[i])))
    })
    
    # output$flow_source <- renderText(paste("Flow Source:", 
    #                                        summary_flow_source))
    # output$flow_sink <- renderText(paste("Flow Sink:",
    #                                      summary_flow_sink))
    # output$flow_type <- renderText(paste("Flow Type:",
    #                                      summary_flow_type))
    # output$subcomponent <- renderText(paste("Subcomponent:", 
    #                                         summary_subcomponent))
    # output$p_subcomponent <- renderText(paste("Partial Subcomponent:",
    #                                           summary_p_subcomponent))
    # output$exact_match <- renderText(paste("Exact Match:",
    #                                        summary_exact_match))
  })
